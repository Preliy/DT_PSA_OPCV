...
install step written into [`_docs/01-setup.md`](_docs/01-setup.md) puts a fact where nothing owns
it.
...
- **`package.json`** holds the version, and nothing else. It is not a Node package: it is
  ...
  semantic-release needs a file to write the version into, and because that version is what
  gets stamped into each module's handoff sidecar as the pairing. **Never edit `version` by
  hand.**
