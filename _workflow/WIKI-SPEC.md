...
**`Home` is whichever file is the repository's landing page**, and which file that is differs:
the main repo opens on its introduction, a module opens on its README. So this is the one entry
every builder must state, and — because a cross-repo link to another repository's landing page has
to resolve too — the one entry every builder must know for its siblings as well.

| Repository | Overrides | What a sibling must know |
|---|---|---|
| main | `_docs/01-introduction.md` → `Home` | `_docs/01-introduction.md` **and** `_docs/README.md` → `Home` |
...
A file that is a real link target but gets no page of its own redirects instead: the main repo's
`_docs/README.md` and every repo's root `README.md` redirect to `Home`, and `CONTRIBUTING.md`
links out to the repository rather than into the wiki.
