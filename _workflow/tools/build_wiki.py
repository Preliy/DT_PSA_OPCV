...
# Home is the repository's landing page. Here, as in every module, that is _docs/README.md
# - the root README.md is the project's welcome page and redirects to it.
OVERRIDES = {
...
def discover():
    ...
    # Globbed, not listed, so adding _docs/08-whatever.md needs no edit here. The
    ...
    if not any(DOCS.glob("[0-9]*.md")):
        raise WikiError("_docs/ has no numbered narrative pages - the wiki's Getting "
                        "started section comes from them. Nothing else can stand in.")
