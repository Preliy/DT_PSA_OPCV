...
into `COMPATIBILITY.md`, which is an instruction the reader cannot follow, in text nobody reviews
before it lands.
...
- **Documentation splits the way authority does.** `_docs/` covers the machine and the twin;
  ...
  step written into `_docs/01-setup.md` is the same mistake as a `.tmc` detail written into
  `_docs/context/`.
...
- **The module pairing is one hand-written field and one derived fact. Never add a version
  range back.** `tested` in `modules.json` says which module version was verified — that is the
  number a person clones. Whether a module is still *current* is decided by comparing its
  committed handoff against `_docs/context/.machine.json`, which is derived and cannot drift.
  There was a `requires` range here and a `requiresMain` range in each module; both were checked
  and **neither ever fired**, because a range admits versions that do not exist yet, so it is
  written wide once and never narrowed. `modules.satisfies()` was deleted with them. A module at a
  version other than `tested` is a note, never a verdict.
- **One tool answers compatibility, and it reads disk-if-cloned, https-if-not.** There were two —
  one for the local table, one for CI — and they could disagree about the same module, which
  needed a paragraph in `COMPATIBILITY.md` explaining which to believe. Do not split them again.
