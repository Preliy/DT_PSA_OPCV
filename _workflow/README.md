...
python _workflow/tools/build_compatibility.py                # refresh the README/COMPATIBILITY table
python _workflow/tools/check_compatibility.py               # ...and check it against the modules' own repos
...
python Beckhoff/_private/tools/build_plc_knowledge.py
python Beckhoff/_private/tools/hmi_check.py
python _workflow/tools/build_compatibility.py
python _workflow/tools/check_compatibility.py
```

The last two are not the same tool asking twice. `build_compatibility.py` writes the table from
the modules **on disk**; `check_compatibility.py` checks the same pairing against what each
module's **own repository** publishes, which is what CI and every reader see. A re-export is
exactly when the two start to disagree — the table says `current` the moment the handoff is
synced locally, and the check keeps saying `behind` until the module commits and pushes it.
...
| `_workflow/tools/` | **Published** — `build_wiki.py`, `publish_wiki.py`, `modules.py`, `build_compatibility.py`, `check_compatibility.py`. They read only tracked files and public URLs, and GitHub Actions has to run them from a checkout of this repo alone | You, when the wiki or pairing contract changes |
